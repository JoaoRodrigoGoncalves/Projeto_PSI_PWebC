...
function apresentar_dados(dados)
{
    $.each(dados.data,function(index,value){
        ...
        $('#tabela_homepage > tbody').append(`<tr><td>${apresentarBotaoFavoritos(value.id)}</td><td>${value.market_cap_rank}</td><td><img class="thumb_img_tabela" src="${img_final}"> <a class="link_tabela" href="./detalhes.html?id=${value.id}">${value.name}</a></td><td class="d-none d-sm-table-cell">${value.symbol}</td><td>${value.current_price}€</td><td>${analise_mudanca_preco(value.price_change_24h, value.price_change_percentage_24h)}</td></tr>`);
    })
}
